...
serve(async (req) => {
  ...
  try {
    ...
    // Upload to Google Drive via shared helper (CX.2 dedup).
    let googleDriveUrl = '';
    try {
      ...
      const { shareUrl } = await uploadToDrive({
        accessToken,
        fileName,
        mimeType: 'text/markdown',
        body: { kind: 'text', data: bundle },
      });
      googleDriveUrl = shareUrl;
    } catch (driveErr) {
      console.warn('Drive upload skipped/failed:', driveErr instanceof Error ? driveErr.message : driveErr);
    }
